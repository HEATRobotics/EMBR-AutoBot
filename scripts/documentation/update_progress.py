...
def render(today: date) -> str:
  ...
  <desc id="desc">{elapsed} of {total} days elapsed from {START} to {END}. As of {as_of}. Calendar time, not engineering completion.</desc>
  ...
  <g font-family="Arial, sans-serif" fill="#ffffff">
    ...
    <text x="24" y="94" font-size="14">{START:%b %d, %Y}</text>
    <text x="696" y="94" text-anchor="end" font-size="14">{END:%b %d, %Y}</text>
    <text x="24" y="117" font-size="12" fill="#c4cdd5">As of {as_of} · {elapsed} / {total} days · Calendar time only</text>
  </g>
</svg>
'''
